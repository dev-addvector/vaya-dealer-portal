...
import { downloadOrder } from '@/api/order.api';
...
// ERP dates are "DD-MM-YYYY HH:MM:SS"; HTML date input gives "YYYY-MM-DD"
...
export default function MyOrdersPage() {
  ...
  const handleDownload = async (po) => {
    ...
    try {
      setDownloading(po);
      const blob = await downloadOrder(po);
  ...
  return (
    <div>
      <div style={breadcrumb.wrap}>
        <div style={container}>
          <span style={breadcrumb.title}>My Orders</span>
        </div>
      </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#fff', fontSize: '14px' }}>
                <thead>
                  ...
                  <tr style={{ backgroundColor: '#fff' }}>
                    ...
                    <td style={filterCell}>
                      <select value={filters.invoiceId} onChange={e => setFilter('invoiceId', e.target.value)} style={filterSelect}>
                        <option value="">Select Invoice Id</option>
                        {invoiceIds.map(v => <option key={v} value={v}>{v}</option>)}
                      </select>
                    </td>
                    <td style={filterCell}>
                      <input type="date" value={filters.invoiceDate} onChange={e => setFilter('invoiceDate', e.target.value)} style={filterInput} />
                    </td>
                    <td style={filterCell}>
                ...
                <tbody>
                  {paged.length === 0 ? (
                    <tr>
                      <td colSpan={10} style={{ ...td, color: '#999', padding: '24px' }}>No orders found.</td>
                    </tr>
                  ) : paged.map((o, i) => (
                    <tr
                      key={o.PONumber ?? i}
                    ...
                    >
                      <td style={td}>{nullStr(o.OrderID)}</td>
                      <td style={td}>{nullStr(o.OrderDate)}</td>
                      <td style={td}>{nullStr(o.InvoiceNo)}</td>
                      <td style={td}>{nullStr(o.InvoiceDate)}</td>
